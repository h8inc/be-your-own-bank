...
export const ProjectionsTab: React.FC<Props> = ({ vaultBal, horizon, data }) => {
  const { corridorChartData, mcBtcChartData, mcBtc } = data;
  ...
  return (
    <>
      ...
      <Card s={{ marginBottom: 12, padding: "16px 8px 8px" }}>
        <Label s={{ marginBottom: 6, paddingLeft: 8 }}>Your BTC value range</Label>
      ...
      <Card s={{ marginBottom: 12, padding: "16px 8px 8px" }}>
        ...
        <div style={{ paddingLeft: 8, marginBottom: 8 }}>
          <span style={{ fontSize: 11, color: c.mute }}>{mcBtc.nPaths.toLocaleString()} simulated futures</span>
        </div>
        <div style={{ width: "100%", height: 200, minWidth: 0 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={mcBtcChartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
              <defs>
              ...
              <XAxis dataKey="year" tick={{ fontSize: 9, fill: c.mute }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 9, fill: c.mute }} tickFormatter={(v) => fmtUsdCompact(v as number)} tickLine={false} axisLine={false} width={48} />
              <Tooltip contentStyle={chartTooltipStyle} formatter={(v) => fmtUsdCompact(v as number)} labelStyle={{ color: c.sub, fontSize: 10 }} />
              <ReferenceLine y={btcPrice} stroke={c.mute} strokeDasharray="3 3" label={{ value: "Today", fill: c.mute, fontSize: 9, position: "insideTopRight" }} />
      ...
      <Expandable title="Full price distribution">
        {[
          { label: "P95 — extreme bull", value: mcBtc.p95, col: c.positive },
          { label: "P90 — optimistic", value: mcBtc.p90, col: c.positive },
          { label: "P75", value: mcBtc.p75, col: c.positive },
          { label: "P50 — median", value: mcBtc.p50, col: c.accent },
          { label: "P25", value: mcBtc.p25, col: c.negative },
          { label: "P10 — conservative", value: mcBtc.p10, col: c.negative },
          { label: "P5 — extreme bear", value: mcBtc.p5, col: c.negative },
        ].map(({ label, value, col }) => (
      ...
      <Expandable title="Risk metrics">
        <Row label="Chance of loss" value={`${(mcBtc.pBelowEntry * 100).toFixed(1)}%`} color={mcBtc.pBelowEntry > 0.3 ? c.negative : c.positive} />
        <Row label="VaR 5%" value={fmtUsdCompact(mcBtc.var5)} color={c.negative} />
        <Row label="CVaR 5%" value={fmtUsdCompact(mcBtc.cvar5)} color={c.negative} />
        <Row label="Typical max drawdown" value={`-${(mcBtc.medianMaxDD * 100).toFixed(0)}%`} color={c.negative} />
        <Row label="Worst-case drawdown (P90)" value={`-${(mcBtc.p90MaxDD * 100).toFixed(0)}%`} color={c.negative} />
